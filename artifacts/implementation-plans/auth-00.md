...
5. Api: AuthController (login/logout) + Api.Tests project
  - Description: Add `Api/Controllers/AuthController.cs` with `POST /api/auth/login` (accepts email, looks up via `AuthService`; on match builds a `ClaimsPrincipal` with `NameIdentifier` = `UserId` and a `Role` claim, then `SignInAsync` the cookie scheme; on no match, fails the request) and `POST /api/auth/logout` (`SignOutAsync`). Add a new `Api.Tests` project (`Microsoft.AspNetCore.Mvc.Testing` + `WebApplicationFactory<Program>`) seeded against an in-memory `ModelDbContext` (`AppExtensions.AddApp` skips registering the SQL Server `DbContext` when `IHostEnvironment.EnvironmentName == "Testing"`, avoiding a dual-provider conflict).
  - Acceptance Criteria: Login with a seeded email issues the auth cookie; login with an unknown email returns 401 with no session; logout clears the session. All covered by `Api.Tests`.
  - Status: Completed
...
7. Api: sample role-restricted endpoint for verification
  ...
  - Acceptance Criteria: Unauthenticated request returns 401 (with a `Location` header pointing at the login page — ASP.NET Core's cookie handler returns status codes rather than a literal redirect for `[ApiController]`/JSON-style requests, reserving real 3xx redirects for browser page navigation); authenticated `User`-role request returns 403 (`Location` pointing at Access Denied); authenticated `Admin`/`Operator` request succeeds (200). Covered by `Api.Tests`.
  - Status: Completed
